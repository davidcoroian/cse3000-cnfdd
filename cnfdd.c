...
// DD
static const bool kDebugPrints_ = true;

static void DebugPrint(const char *info) {
  if (kDebugPrints_) {
    fprintf(stdout, "[DEBUG-cnfdd] %s\n", info);
    fflush(stdout);
  }
}
// --- DD
...
static int run(const char *name) {
  ...
  // DD DEBUG PRINTS
  fprintf(stdout, "[DEBUG-cnfdd] DOING A CALL #%d\n", calls);
  fflush(stdout);

  sprintf(buffer, "exec %s %s", cmd, name);
  res = maskstatus(system(buffer));
  free(buffer);

  fprintf(stdout, "[DEBUG-cnfdd] RESULT %d\n", res);
  fflush(stdout);
...
static void setup(int compute_expected) {
  ...
  // FILE *t = fopen("~/cse3000-how-to-break-a-solver/david-delta-debugger/"
  //                 "cnfuzzdd2013/cnff/out.txt",
  //                 'w');
  // fprintf(t, "%u", tmp);
}
...
static void reduce(void) {
  ...
  width = size_clauses; // DD TEMP maybe /2????
  total = 0;
  ...
  while (width) {
    ...
    do {
      ...
      if (isatty(2)) {
        fprintf(stderr,
                "[cnfdd] reduce(%d) width %d removed %d iterator at %d/%d\r",
      ...
      if (found) {
        print(tmp);
        if (run(tmp) == expected) {
          DebugPrint("Expected result during run! - bug present");
          ...
          DebugPrint("NOT expected result! - bug missing");
    ...
    if (removed) {
      DebugPrint("Removed some clauses, saving result!");
...
static void move(void) {
  ...
  // Timeouts and software execution are non-deterministic.
